(() => {
  ...
  function calcRoof(){
    ...
    if(sys === 'desmontable60' || sys === 'desmontable120'){
      ...
      if(sys === 'double'){ title = 'Techo doble TC47'; rows.push(['TC47 primario', Math.ceil(A / 2.7) + ' barras', 'UNE 102043 · primaria'], ['TC47 secundario', Math.ceil(A / 1.5) + ' barras', 'UNE 102043 · secundaria'], ['Crucetas', Math.ceil(A / .475) + ' uds', 'Cruces primaria-secundaria'], ['Horquillas', Math.ceil(A / .95) + ' uds', 'Suspensiones'], ['Varilla M6 1 m', Math.ceil(A / .95 * Math.max(.05, d / 100)) + ' uds', 'Plenum ' + d + ' cm']); }
      else if(sys === 'simple'){ title = 'Techo TC47 simple'; rows.push(['TC47 portante', Math.ceil(A / 1.5) + ' barras', 'UNE 102043 · estructura'], ['Horquillas', Math.ceil(A / 1.2) + ' uds', 'Suspensiones'], ['Varilla M6 1 m', Math.ceil(A / 1.2 * Math.max(.05, d / 100)) + ' uds', 'Plenum ' + d + ' cm']); }
      else if(sys === 'sierra'){ title = 'Techo perfil sierra'; rows.push(['Perfil sierra', Math.ceil(A / 2.7) + ' barras', 'UNE 102043 · primario'], ['TC47 secundario', Math.ceil(A / 1.5) + ' barras', 'Secundario'], ['Suspensiones', Math.ceil(A / .95) + ' uds', 'Puntos de suspensión']); }
      else { title = 'Techo canal + montante'; rows.push(['Canal R70', Math.ceil(2 * (L + W) / 3) + ' barras', 'UNE-EN 14195 · perímetro'], ['Montante M70', Math.ceil(A / 1.8) + ' barras', 'UNE-EN 14195 · portante'], ['Suspensiones MS', Math.ceil(A / 2.7) + ' uds', 'Puntos de suspensión']); }
